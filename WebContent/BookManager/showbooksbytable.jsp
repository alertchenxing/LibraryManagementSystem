...
function trclick(obj){
	...
	clickedRow = obj;
	var booknum = clickedRow.cells[0].innerText;
	window.parent.location.href="../SelectOneBookServlet?booknum="+booknum;
}
...
<body >
	...
	<%
		...
		booksList = (ArrayList)session.getAttribute("booksArrayList");
...
<div class="box">
	...
	<form action="../SelectfromresultServlet" method="get" onsubmit="return checkSelect()" target="bookinfobyselect">
		<p>
			...
			<input type="submit" value="结果中检索" class="button">
		</p>
	</form></div>
	<div class="showtb">
		<table>
	...
	<%
			...
			intRowCount = booksList.size();//计算记录总数
			...
			if(intPageCount > 0){
				...
				int temp = i;
				while(i < temp + intPageSize && i < booksList.size()){
					BookBean bookBean = (BookBean)booksList.get(i);
	%>
					<tr onclick="trclick(this)" onmouseover="trhover(this)" onmouseout="trout(this)">
							<td><%=bookBean.getBookNumm() %></td>
							<td><%=bookBean.getBookName() %></td>
							<td><%=bookBean.getBookType() %></td>
							<%
								if(bookBean.getBookTrans() != null){%>
								<td><%=bookBean.getBookWriter() %>/<%=bookBean.getBookTrans() %></td>
							<%		
								}else{%>
									<td><%=bookBean.getBookWriter() %>/</td>
							<%		
								}
							%>
							<td><%=bookBean.getBookPublishr() %></td>
					</tr>
	<%
					i++;
				}
				if(intPage == intPageCount){
					int flag = booksList.size() % intPageSize;
					if(flag != 0) {
			...
			function setpage(){
				if(toltalpage <= pagesize){//如果总页数小于等于10页
					for(count = 1; count <= toltalpage; count ++){
						if(count != cpage){//跳转页数不等于当前页数
							outstr = outstr + "<li><a href='showbooksbytable.jsp?page="+count+"' onclick='gotopage("+count+")'>"+count+"</a></li>";
						}else{
				...
				if(toltalpage > pagesize){//总数大于10
					if(parseInt((cpage - 1) / pagesize) == 0){//小于10页显示
						...
						outstr = outstr + "<li><a href='showbooksbytable.jsp?page="+count+"' onclick='gotopage("+count+")'>>></a></li>";
					}else if(parseInt((cpage - 1) / pagesize) == parseInt(toltalpage/ pagesize)){//最后几页显示
						outstr = outstr + "<li><a href='showbooksbytable.jsp?page="+(parseInt((cpage - 1) / 10) * 10)+"' onclick='gotopage("+(parseInt((cpage - 1) / 10) * 10)+")'><<</a></li>";
						...
						outstr = outstr + "<li><a href='showbooksbytable.jsp?page="+(parseInt((cpage - 1) / pagesize) * pagesize)+"' onclick='gotopage("+(parseInt((cpage - 1) / pagesize) * pagesize)+")'><<</a></li>";
				...
				document.getElementById("page").innerHTML = "<div id='setpage'><span id='info'>共"+toltalpage+"页|第"+cpage+"页</span><ul class='pagination'>"+outstr+"</ul><form action='showbooksbytable.jsp' method='get'"+
